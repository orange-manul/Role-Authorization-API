...
class ControllerExample extends Controller
{
    ...
    public function post(Request $request): JsonResponse
    {
        if ($this->isAdmin($request->user())) {
            $posts = Post::all();
            return $this->onSucces($posts->toArray(), 'Posts Retrieved');
        }
    ...
    public function singlePost(Request $request, $id): JsonResponse
    {
        ...
        if ($this->isAdmin($user) || $this->isWriter($user) || $this->isSubscriber($user)) {
            ...
            if (!empty($post)) {
                return $this->onSucces($post, 'Post Retrieved');
            }
    ...
    public function createPost(Request $request): JsonResponse
    {
        ...
        if ($this->isAdmin($user) || $this->isWriter($user)) {
            ...
            if ($validator->passes()) {
                ...
                $post->save();
                return $this->onSucces($post, 'Post Created');
            }
    ...
    public function updatePost(Request $request, $id): JsonResponse
    {
        ...
        if ($this->isAdmin($user) || $this->isWriter($user)) {
            ...
            if ($validator->passes()) {
                ...
                $post->content = $request->input('content');
                $post->save();
                return $this->onSucces($post, 'Post Updated');
            }
            return $this->onError(400, $validator->errors());
        }
        return $this->onError(401, 'Unauthorized Access');
    ...
    public function deletePost(Request $request, $id): JsonResponse
    {
        ...
        if ($this->isAdmin($user) || $this->isWriter($user)) {
            ...
            if (!empty($post)) {
                return $this->onSucces($post, 'Post Deleted');
            }
            return $this->onError(404, 'Post Not Found');
        }
        return $this->onError(401, 'Unauthorized Access');
    ...
    public function createWriter(Request $request): JsonResponse
    {
        ...
        if ($this->isAdmin($user)) {
            $validator = Validator::make($request->all(), $this->userValidateRules());
            if ($validator->passes()) {
                ...
                return $this->onSucces($writerToken, 'User Created With Writer Privilege');
            }
            return $this->onError(400, $validator->errors());
        }
        return $this->onError(401, 'Unauthorized Access');
    ...
    public function createSubscriber(Request $request): JsonResponse
    {
        ...
        if ($this->isAdmin($user)) {
            ...
            if ($validator->passes()) {
                ...
                return $this->onSucces($writerToken, 'User Created With Subscriber Privilege');
            }
            return $this->onError(400, $validator->errors());
        }
        return $this->onError(401, 'Unauthorized Access');
    ...
    public function deleteUser(Request $request, $id): JsonResponse
    {
        ...
        if ($this->isAdmin($user)) {
            ...
            if ($user->role !== 1) {
                ...
                if (!empty($user)) {
                    return $this->onSucces('', 'User Deleted');
                }
